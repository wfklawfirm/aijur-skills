...
| `evaluations` | A rubric-scored judgement on an attempt or simulation session: per-criterion `criteria` JSON (each with quoted evidence), strengths/mistakes, `humanReviewStatus`, `humanReviewReason` (`unverified_evidence`\|`low_confidence`\|`capped_by_critical_mistake`\|`incomplete_rubric_coverage`\|`null` — why `verifyEvaluation()` queued this one, computed at evaluation time and rendered in `admin/review-queue/page.tsx` so a reviewer isn't guessing; `null` for `not_required` and for rows written before this column existed). Also carries `pendingMastery` (the skill(s)/target level/depth this score is evidence for, captured at evaluation time) and `masteryApplied` (false until that evidence has actually been folded into `masteryRecords` — immediately for `not_required`, only after a reviewer upholds/edits a `queued` evaluation; see `AI_ARCHITECTURE.md`). |
