...
| **Adaptive Professional Journey Engine (Phase 1: personalized Hooks)** | Functional, scoped | `src/lib/adaptive/{fingerprint,quality-gates,hook-composer,hooks}.ts` + `src/lib/ai/agents/adaptive-hook.ts` + `content/adaptive/dimensions.ts`. A 3-layer hybrid content model (expert-approved skills → deterministic bilingual composer templates → optional AI personalization, same offline-first `runAgent()` pattern as every other agent) generates short, personalized "hook" prompts on the Home page, gated by a real offline novelty-detection system (structural-key + MinHash-lite lexical fingerprinting, `NOVELTY_THRESHOLD=0.45`) and 6 mechanical quality gates before anything is shown. A Content Reservoir (`adaptiveContent` table) is checked before any generation happens; `scripts/generate-adaptive-content.ts` pregenerates it (`npm run generate:adaptive`, confirmed idempotent). Per-user exposure tracking (`userContentExposure`) drives repetition avoidance. Admin monitor at `/admin/adaptive-content` (stats, hook-type/skill coverage, human-review queue with approve/reject). **This is a scoped Phase 1 of a much larger 46-section specification** — see `docs/ADAPTIVE_ENGINE_ARCHITECTURE.md` for the full built-vs-deferred mapping (8 of 10 named hook types, 6 of 12 named quality gates, lexical/structural novelty proxy rather than true semantic embeddings, hooks only — not roleplays/missions/voice/visual/Legal-English variants yet). Reservoir currently holds 108 published hooks (9 skills × 2 locales × 6 combos), average novelty 0.92 (range 0.6875–1.0), average quality score 1.0 — real numbers queried from the dev DB, not estimates. Covered by `tests/adaptive-content.test.ts` (fingerprinting, novelty, quality gates, offline composer — 5 suites). |
| **i18n** | Functional, AR/EN only | `src/lib/i18n/config.ts`: `LOCALES = ["ar","en"]`, `DEFAULT_LOCALE = "ar"`, and an explicit `PLANNED_LOCALES = ["fr"]` — French is a stated intent, not built. Proxy-based locale routing (`src/proxy.ts`) with cookie > Accept-Language > Arabic-default resolution. **A global language switcher (`src/components/layout/language-switcher.tsx`) now appears on every page** — via `AppHeader` for all 9+ authenticated screens (home, learn, practice, progress, profile, unit player, simulation, every admin screen) and added individually to every standalone pre-auth screen (landing, sign-in, sign-up, forgot/reset password, verify-email) and the diagnostic's intro screen (onboarding already had its own). Fixed a real, previously-dead mechanism: `src/proxy.ts` has read an `aijur_locale` cookie for its cookie>Accept-Language>default resolution since it was written, but nothing ever set that cookie — a returning visit to a locale-less URL always re-guessed. The switcher now writes it (1-year expiry) on every language change, including from onboarding's own switcher. `tests/i18n-parity.test.ts` checks dictionary key parity between `ar.ts`/`en.ts`; `tests/e2e/rtl.spec.ts` has 3 new tests confirming the switcher actually navigates, flips `dir`, and that the cookie is honoured on a later locale-less visit. |
...
- **Adaptive engine beyond Phase 1**: true semantic-embedding similarity (a lexical/
  structural proxy is used instead — see `docs/ADAPTIVE_ENGINE_ARCHITECTURE.md` §5 for
  why); content types beyond Hooks (icebreakers, daily challenges as a distinct type,
  scenario variations, AI roleplays, voice/visual exercises, Legal English activities,
  missions, feedback messages, stage summaries, weekly reviews, recaps, next-mission
  previews); source/book-grounded generation via a knowledge graph; an AI-judge review
  pass (Phase 1's 6 quality gates are mechanical checks only, no model-based review);
  cross-user/cohort diversity balancing (exposure tracking is per-user only); the
  spec's 4-panel admin dashboard (Phase 1 ships one consolidated monitor page). Content
  generation is **not** unlimited or never-repeating — the reservoir is a fixed,
  countable set of rows, currently 108, expanded by re-running a pregeneration script.
