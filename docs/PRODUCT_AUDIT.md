...
| **RBAC / multi-tenancy** | Functional | `src/lib/auth/rbac.ts` defines 11 permissions, 4 system roles (learner/author/reviewer/admin) and 5 org roles (owner/admin/manager/author/member); `assertTenant()` enforces org isolation on every scoped query. Schema has `organizations`, `memberships`, `teams` tables. `assertTenant()` now has real call sites — the org-admin surface (`src/lib/actions/org-core.ts`: list/add/role-change/remove members, an org report) calls `require_()` + `assertTenant()` before every query; the `org.assign` permission (team/competency-profile assignment) still has no action surface. Covered by `tests/rbac.test.ts` (the primitive, mock users) and `tests/org-tenant-isolation.test.ts` (the real action surface, two seeded organizations, 9 tests). |
